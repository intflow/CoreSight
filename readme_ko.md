...
#### 문제점
- **텍스트 임베딩 한계**: 텍스트 임베딩을 사용하여 쥐의 손 특징을 추출하는 것은 사실상 불가능한 수준
- **비디오 임베딩 제약**: Zero-shot 비디오 임베딩 방법으로는 이런 특정 데이터가 존재할 리 만무

#### 해결책: Few-Shot 학습
- **방법론**: 최소한의 데이터로 few-shot 학습을 구현하여 임베딩 추출
- **데이터셋 크기**: 쥐가 벽을 터치하는 이벤트 약 10개 샘플
- **라벨링 전략**: 
  - 벽 접촉 이벤트 시 쥐의 발만 라벨링 진행
  - 다른 클래스에 악영향을 줄 수 있어 쥐 몸체는 라벨링하지 않음

![annotation_sample](asset/rat/annotation_sample.png)
- **효율성**: 전체 라벨링 과정이 2-3분만에 완료 (단일 클래스)

### VP Detector 및 Object Tracker 구현

#### VP Detector (임베딩 추출)
- **VPE 갱신 제거**: 
  - 문제: 64배치 이미지 안에 발이 포함되지 않을 경우, 임베딩 추출을 하지 못함
  - 영향: bbox 검출 없이는 임베딩 갱신이 발생하지 않음
  - 필요한 해결책: 이미지-bbox-클래스 매칭 기반 조건부 갱신 구현

#### Object Tracker (로그 추출)
- **오탐률**: 높은 오탐률 관찰됨
- **신뢰도 점수**: 대부분의 예측값이 0.01 이하의 신뢰도 값
- **초기값**: 초기 모델 파라미터가 가장 정확한 결과 제공

### 분석 결과

#### 방법론
- **시간 분할**: frame_time을 10초 단위로 분할
- **기준점 설정**: 플라스크 중심 위치를 (846, 676)으로 수동 설정
- **AI 협업 분석**: Chat 기반 분석 방법론 개발
...
#### 실험 결과
- **분석 시간**: 30분 분석 세션
- **후보 이벤트**: 19개의 잠재적 터치 이벤트 식별
- **확인된 터치**: 8개의 검증된 벽 접촉 사례

**하이라이트 영상:**

![하이라이트 샘플 01](asset/rat/highlight_sample_01.gif)

![하이라이트 샘플 02](asset/rat/highlight_sample_02.gif)
...
#### 해결책: Few-Shot 학습
- **방법론**: 최소한의 데이터로 few-shot 학습을 구현하여 임베딩 추출
- **데이터셋 크기**: 돼지 개체 감지 약 4개 샘플
- **적용 범위**: 돼지 개체 1개만 분석 적용
- **라벨링 전략**: 
  - 돼지 전체 개체를 라벨링하여 위치 추적
...
#### 급수기 영역 설정
- **고객 요구사항**: 어느 급수기를 많이 애용하는지 분석
- **영역 설정**: 급수기별 접근 영역 수동 설정

![set_water](asset/pig/set_water.png)

#### Object Tracker (로그 추출)
- **접근 감지**: 돼지가 급수기 영역에 접근 시 로그 기록
- **로그 형식**: 접근 시 1(접근), 미접근 시 0으로 컬럼 추출
- **추적 대상**: 단일 돼지 개체의 급수기별 접근 패턴

### 분석 결과

#### 방법론
- **시간 분할**: 전체 데이터를 30개 구간으로 분할
- **AI 협업 분석**: Chat 기반 점유율 계산 방법론 개발
...
![MOVA0030 활발 구간 샘플](asset/pig/MOVA0030_sample.gif)

**급수 저조 구간:**

![MOVA0008 저조 구간 샘플](asset/pig/MOVA0008_sample.gif)
