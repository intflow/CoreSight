...
#### Challenges
- **Text Embedding Limitations**: Extracting rat paw features using text embedding is practically impossible
- **Video Embedding Constraints**: Zero-shot video embedding methods cannot handle such specific data

#### Solution: Few-Shot Learning
- **Methodology**: Implemented few-shot learning with minimal data to extract embeddings
- **Dataset Size**: Approximately 10 samples of rat wall-touching events
- **Labeling Strategy**: 
  - Only labeled rat paws during wall contact events
  - Excluded rat body labeling to prevent negative impact on other classes

![annotation_sample](asset/rat/annotation_sample.png)
- **Efficiency**: Complete labeling process takes only 2-3 minutes (single class)

### VP Detector and Object Tracker Implementation

#### VP Detector (Embedding Extraction)
- **VPE Update Removal**: 
  - Problem: When paws are not included in 64-batch images, embedding extraction fails
  - Impact: No embedding updates occur without bbox detection
  - Required Solution: Implement conditional updating based on image-bbox-class matching

#### Object Tracker (Log Extraction)
- **False Positive Rate**: High false positive rate observed
- **Confidence Scores**: Most predictions show confidence values below 0.01
- **Initial Values**: Initial model parameters provide the most accurate results

### Analysis Results

#### Methodology
- **Time Segmentation**: Divided frame_time into 10-second intervals
- **Reference Point Setting**: Manually set flask center position at (846, 676)
- **AI Collaborative Analysis**: Chat-based analysis methodology development
...
- **Analysis Duration**: 30-minute analysis session
- **Candidate Events**: 19 potential touch events identified
- **Confirmed Touches**: 8 verified wall contact instances

**Highlight Videos:**

![Highlight Sample 01](asset/rat/highlight_sample_01.gif)

![Highlight Sample 02](asset/rat/highlight_sample_02.gif)
...
#### Solution: Few-Shot Learning
- **Methodology**: Implemented few-shot learning with minimal data to extract embeddings
- **Dataset Size**: Approximately 4 samples for pig entity detection
- **Application Scope**: Analysis applied to only 1 pig entity
- **Labeling Strategy**: 
  - Labeled entire pig entity for location tracking

![annotation_pig](asset/pig/annotation_pig.png)

### VP Detector and Object Tracker Implementation

#### Water Dispenser Area Setting
- **Client Requirement**: Analyze which water dispenser is used most frequently
- **Area Setting**: Manual configuration of approach zones for each water dispenser

![set_water](asset/pig/set_water.png)

#### Object Tracker (Log Extraction)
- **Approach Detection**: Record logs when pig approaches water dispenser areas
- **Log Format**: Extract column with 1 (approach) when approaching, 0 when not
- **Tracking Target**: Single pig entity's approach patterns to water dispensers

### Analysis Results

#### Methodology
- **Time Segmentation**: Divided entire data into 30 time periods
- **AI Collaborative Analysis**: Chat-based occupancy rate calculation methodology development
...
- **Most Preferred Water Dispenser**: Dispenser 3 (77% occupancy rate)

**Summary Results:**

> **Conclusion**: The water dispenser that pigs use most frequently is Dispenser 3 (water_area3), with an occupancy rate of approximately 77%.
> 
...
> - Dispenser 4: 12%
> 
 > **Therefore, Dispenser 3 has the highest occupancy rate and is overwhelmingly the most frequently used.**
...
**Active Water Consumption Periods:**

![MOVA0030 Active Period Sample](asset/pig/MOVA0030_sample.gif)

**Low Water Consumption Periods:**

![MOVA0008 Low Activity Period Sample](asset/pig/MOVA0008_sample.gif)
